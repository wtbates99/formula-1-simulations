...
class JolpicaClient:
    ...
    def fetch_all(self, endpoint: str, page_size: int = 1000) -> dict[str, Any]:
        """Fetch a full paginated Ergast/Jolpica endpoint."""
        offset = 0
        ...
        while True:
            ...
            if merged_payload is None:
                ...
                target = self._dig(merged_payload["MRData"], key_path)
                incoming = self._dig(mr_data, key_path)
    ...
    def _request(self, endpoint: str, limit: int, offset: int) -> dict[str, Any]:
        ...
        retriable_errors = (urllib.error.HTTPError, urllib.error.URLError, TimeoutError)
    ...
    def _should_give_up_http(exc: Exception) -> bool:
        if not isinstance(exc, urllib.error.HTTPError):
            return False
        # Retry the status codes that are typically transient.
        return exc.code not in (408, 425, 429, 500, 502, 503, 504)

    @staticmethod
    def _locate_list_path(mr_data: dict[str, Any]) -> tuple[str, ...]:
        # Ergast responses put the list under one nested "Table" object.
        table_key = next((k for k in mr_data.keys() if k.endswith("Table")), None)
        if table_key is None:
            raise ValueError(f"Could not find table key in response: {mr_data.keys()}")
        table = mr_data[table_key]
        list_key = next((k for k, v in table.items() if isinstance(v, list)), None)
        if list_key is None:
            raise ValueError(f"Could not find list payload in table: {table.keys()}")
    ...
    def _dig(payload: dict[str, Any], path: tuple[str, ...]) -> list[Any]:
        ...
        if not isinstance(cur, list):
            raise TypeError(f"Expected list at path {path}, got {type(cur)}")
        return cur
...
class F1Warehouse:
        ...
        self.conn.executescript(
            ...
            WITH pit_agg AS (
                SELECT
                    ...
                    SUM(duration_millis) AS pit_total_duration_millis
                FROM pit_stops
            ...
            lap_agg AS (
                SELECT
                    race_id,
                    driver_id,
                    AVG(time_millis) AS avg_lap_time_millis,
                    MIN(time_millis) AS best_lap_time_millis
                FROM lap_times
                GROUP BY race_id, driver_id
            ),
            base AS (
                SELECT
                    ...
                    rr.points,
                    rr.laps AS laps_completed,
                    rr.race_time_millis,
                    rr.status_id,
                    cs.position AS constructor_standing_position,
                    cs.points AS constructor_standing_points,
                    ds.position AS driver_standing_position,
                    ds.points AS driver_standing_points,
                    COALESCE(pa.pit_stop_count, 0) AS pit_stop_count,
                    pa.pit_total_duration_millis,
                    la.avg_lap_time_millis,
                    la.best_lap_time_millis,
            ...
            SELECT
                b.*,
                AVG(b.finish_position) OVER (
                    PARTITION BY b.driver_id
                    ORDER BY b.season_year, b.round
                    ROWS BETWEEN 3 PRECEDING AND 1 PRECEDING
                ) AS rolling3_avg_finish_position,
                AVG(b.points) OVER (
                    PARTITION BY b.driver_id
                    ORDER BY b.season_year, b.round
                    ROWS BETWEEN 3 PRECEDING AND 1 PRECEDING
                ) AS rolling3_avg_points,
                AVG(b.finish_position) OVER (
                    PARTITION BY b.driver_id
                    ORDER BY b.season_year, b.round
                    ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
                ) AS rolling5_avg_finish_position,
                AVG(b.points) OVER (
                    PARTITION BY b.driver_id
                    ORDER BY b.season_year, b.round
                    ROWS BETWEEN 5 PRECEDING AND 1 PRECEDING
                ) AS rolling5_avg_points,
    ...
    def upsert_season(self, year: int) -> None:
        self.conn.execute(
            "INSERT OR IGNORE INTO seasons(season_year) VALUES (?)",
            (year,),
        )
    ...
    def upsert_race(self, race: dict[str, Any], season_year: int) -> int:
        ...
        if row is None:
            raise RuntimeError(f"Could not find race id for {season_year}/{race.get('round')}")
        return int(row[0])
    ...
    def upsert_status(self, status: str) -> int:
        row = self.conn.execute(
            "SELECT status_id FROM status_codes WHERE status = ?",
            (status,),
        ).fetchone()
        if row is not None:
            return int(row[0])

        row2 = self.conn.execute("SELECT COALESCE(MAX(status_id), 0) + 1 FROM status_codes").fetchone()
        status_id = int(row2[0]) if row2 is not None else 1
        self.conn.execute(
            "INSERT INTO status_codes(status_id, status) VALUES (?, ?)",
            (status_id, status),
        )
        return status_id
    ...
    def upsert_race_result(self, race_id: int, result: dict[str, Any]) -> None:
        ...
        self.conn.execute(
            ...
            INSERT INTO race_results(
                ...
                position_order, points, laps, status_id, race_time_millis, race_time_text,
                fastest_lap_rank, fastest_lap_number, fastest_lap_time, fastest_lap_avg_speed_kph
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(race_id, driver_id) DO UPDATE SET
                ...
                race_time_text=excluded.race_time_text,
    ...
    def rebuild_feature_table(self) -> None:
        self.conn.execute("DELETE FROM driver_race_features")
        self.conn.execute(
            """
            INSERT INTO driver_race_features(
                race_id, driver_id, constructor_id, season_year, round, grid, finish_position, points,
                laps_completed, race_time_millis, status_id, constructor_standing_position,
                constructor_standing_points, driver_standing_position, driver_standing_points,
                pit_stop_count, pit_total_duration_millis, avg_lap_time_millis, best_lap_time_millis,
                rolling3_avg_finish_position, rolling3_avg_points, rolling5_avg_finish_position,
                rolling5_avg_points, age_days, target_finish_position, target_points,
...
def sync_history(config: SyncConfig) -> None:
    ...
    try:
        for year in range(config.from_year, config.to_year + 1):
            ...
            season_payload = client.fetch_all(f"{year}")
            races = season_payload["MRData"]["RaceTable"].get("Races", [])
            if not races:
            ...
            for race in races:
                ...
                _sync_results(client, warehouse, year, round_no, race_id)
                _sync_qualifying(client, warehouse, year, round_no, race_id)
                _sync_sprint(client, warehouse, year, round_no, race_id)
                _sync_driver_standings(client, warehouse, year, round_no, race_id)
                _sync_constructor_standings(client, warehouse, year, round_no, race_id)

                if config.include_lap_times:
                    _sync_lap_times(client, warehouse, year, round_no, race_id)
                if config.include_pit_stops:
                    _sync_pit_stops(client, warehouse, year, round_no, race_id)

            warehouse.commit()
            print(f"[{year}] done", flush=True)
        if config.rebuild_features:
            print("rebuilding feature table", flush=True)
            warehouse.rebuild_feature_table()
            print("feature table ready", flush=True)
    finally:
        warehouse.close()
...
def _sync_driver_standings(
    client: JolpicaClient, warehouse: F1Warehouse, year: int, round_no: str, race_id: int
) -> None:
...
def _sync_constructor_standings(
    client: JolpicaClient, warehouse: F1Warehouse, year: int, round_no: str, race_id: int
) -> None:
...
def _mmss_to_millis(text: str | None) -> int | None:
    ...
    try:
        if len(parts) == 2:
            ...
            return int((minutes * 60 + sec_part) * 1000)
        sec_part = float(parts[0])
        return int(sec_part * 1000)
    except (TypeError, ValueError):
        return None
...
def parse_args() -> SyncConfig:
    parser = argparse.ArgumentParser(
        description="Build a local SQLite warehouse for historical F1 data (Ergast/Jolpica)."
    )
    parser.add_argument(
        "--db",
        default="f1_history.db",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Max request retries for transient network errors",
    )
    parser.add_argument(
        "--skip-lap-times",
        action="store_true",
        help="Skip per-lap timing data ingestion",
    )
    parser.add_argument(
        "--skip-pit-stops",
        action="store_true",
        help="Skip pit stop data ingestion",
    )
    parser.add_argument(
        "--skip-feature-rebuild",
        action="store_true",
        help="Skip rebuilding materialized driver_race_features table",
    )
